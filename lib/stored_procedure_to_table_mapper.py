...
class StoredProcedureToTableMapper:
    ...
    def _execute_mapping(self, ddl_df):
        ...
        output_df = pd.DataFrame(columns=['table_name', 'sql_operation', 'procedure_name'])
        
        for procedure_name in procedure_names:
            
            sql_code = procedures_ds[procedures_ds['db_object_name'] == procedure_name]['sql_code'].values[0]
            procedure_code = self.sql_code_parser._extract_procedure_declaration_from_code(procedure_name, sql_code)
